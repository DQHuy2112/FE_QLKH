...
export default function InternalExportReceiptsPage() {
    ...
    const [targetStoreFilter, setTargetStoreFilter] = useState<number | 'ALL'>('ALL');
    ...
    const loadExports = async () => {
        try {
            ...
            // Apply target store filter
            const filtered = targetStoreFilter === 'ALL'
                ? exports
                : exports.filter(ex => ex.targetStoreId === targetStoreFilter);
            ...
            if (err instanceof Error) {
                ...
                setError('Lỗi tải danh sách phiếu xuất nội bộ');
            }
            console.error(err);
        } finally {
            setLoading(false);
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    value={targetStoreFilter === 'ALL' ? 'ALL' : String(targetStoreFilter)}
                                    onChange={(e) => setTargetStoreFilter(e.target.value === 'ALL' ? 'ALL' : Number(e.target.value))}
                    ...
                    <table className="w-full table-fixed">
                        ...
                        <thead>
                            <tr className="bg-[#0046ff] text-white h-[48px]">
                                ...
                                <th className="px-4 text-center font-bold text-sm">Kho đích</th>
                            ...
                            {data.length === 0 ? (
                                ...
                                currentData.map((record, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 text-center text-sm">
                                            {supplierMap.get(record.targetStoreId) || record.targetStoreName || `Kho #${record.targetStoreId}`}
                                        </td>
                                        <td className="px-4 text-center text-sm">
