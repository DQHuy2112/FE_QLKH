...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.success) {
        ...
        // if account locked message, show unlock modal
        if ((res.message || '').toLowerCase().includes('khóa')) {
  ...
  const handleVerifyUnlock = async () => {
    ...
    try {
      const { success, message } = await import('@/services/auth.service').then(m => m.verifyUnlockCode(username, unlockCode));
      // verifyUnlockCode returns ApiResponse; but direct import used above
      // call via service
      const res = await import('@/services/auth.service').then(m => m.verifyUnlockCode(username, unlockCode));
      if (!res.success) {
      ...
      setError('Mở khóa thành công. Vui lòng đăng nhập lại.');
    } catch (err: unknown) {
